...
class OversightProbeTemplate(models.Model):
    ...
    probe_type = fields.Selection(
        selection=_SELECTION_PROBE_TYPE, string="Type", required=True, readonly=True
    )

    cron_id = fields.Many2one(comodel_name="ir.cron", copy=False, readonly=True)
    ...
    interval_type = fields.Selection(
        selection=_SELECTION_INTERVAL_TYPE,
        default="minutes",
        required=True,
    )
    ...
    check_ids = fields.One2many(
        string="Checks",
        comodel_name="oversight.check",
        inverse_name="probe_template_id",
    )

    check_qty = fields.Integer(string="Checks Qty", compute="_compute_check_qty")

    last_check_id = fields.Many2one(
        comodel_name="oversight.check", string="Last Check", readonly=True, copy=False
    )

    last_check_state = fields.Selection(
        selection=_SELECTION_LAST_CHECK_STATE,
        readonly=True,
        compute="_compute_last_check",
        store=True,
    )
    ...
    alert_ids = fields.One2many(
        comodel_name="oversight.probe.alert",
        string="Alerts",
        inverse_name="probe_template_id",
    )
    ...
    def _compute_last_check(self):
        for template in self.filtered(lambda x: x.last_check_id):
            ...
            template.last_value_string = template._get_variant()._get_value_string(
    ...
    def _compute_value_type(self):
        for template in self:
            template.value_type = template._get_variant()._variant_value_type
    ...
    def _get_variant_model(self):
        self.ensure_one()
        return self.env[f"oversight.probe.variant.{self.probe_type}"]
