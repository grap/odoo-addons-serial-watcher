...
class OversightCheck(models.Model):
    ...
    state = fields.Selection(selection=_SELECTION_STATE, required=True, readonly=True)
    ...
    value_integer = fields.Integer(string="Value (Integer)", readonly=True)

    value_text = fields.Char(string="Value (Text)", readonly=True)

    value_float = fields.Float(string="Value (Float)", readonly=True, default=-1)
